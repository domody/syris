...
def add_message(
    chat_id: int, message: NewMessage, db: sqlite3.Connection = Depends(get_db)
):
    cur = db.cursor()
    cur.execute(
        """
...
def rename_chat(
    chat_id: int, body: RenameChatBody, db: sqlite3.Connection = Depends(get_db)
):
    cur = db.cursor()
    cur.execute(
        """
...
def delete_chat(chat_id: int, db: sqlite3.Connection = Depends(get_db)):
    cur = db.cursor()
    cur.execute(
        """
