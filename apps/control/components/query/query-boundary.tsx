...
export function QueryBoundary({
  ...
  if (isError && !hasData) {
    if (isNetworkError(error)) {
      return (
        <>
          {offline ?? (
            <div className="p-3 rounded-md border text-sm opacity-80">
              API unavailable (server down / blocked / offline). You can still
              navigate the app.
  ...
  // If we have data but a background refetch fails, you often still want to show the stale data.
