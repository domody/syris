...
const SPARKLINE_DATA = [8, 12, 10, 14, 18, 22, 19, 16, 20, 28, 34, 32, 30, 25, 22, 28, 36, 44, 40, 36, 30, 24, 28, 33];
...
function AutonomyPill({ level }: { level: string | null }) {
  ...
  return (
    <View className="flex-row items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/15 dark:bg-blue-400/15">
      <Text className="text-blue-600 dark:text-blue-400 text-sm font-semibold font-mono">{level}</Text>
      <Text className="text-zinc-500 dark:text-zinc-400 text-xs">scoped autonomy</Text>
    </View>
  );
}
...
export default function OverviewScreen() {
  ...
  const healthDotVariant =
    systemHealth === 'critical' ? 'error' : systemHealth === 'degraded' ? 'warning' : 'success';
    ...
    <SafeAreaView className="flex-1 bg-background">
      ...
      >
        ...
        <View className="flex-row items-center justify-between pt-3 pb-4">
          <View>
            <Text className="text-2xl font-semibold tracking-tight text-foreground">Overview</Text>
          </View>
        ...
        <Card>
          <View className="flex-row justify-between items-start">
            <View className="flex-1">
              ...
              <Text className="text-xs text-muted mt-1 font-mono">
                14d 03:22:07 · 7 subsystems reachable
              </Text>
            </View>
          </View>
          ...
          <View className="mt-3.5">
            <View className="flex-row justify-between mb-1">
              <Text className="text-[10px] font-mono text-muted tracking-wider">PIPELINE · 24H</Text>
              <Text className="text-[10px] font-mono text-muted">1,284 events · 3 errors</Text>
            </View>
        ...
        <Pressable className="bg-blue-500/10 dark:bg-blue-400/10 border border-blue-500/20 dark:border-blue-400/20 rounded-xl p-4 active:opacity-70">
          <View className="flex-row justify-between items-center mb-2">
            ...
            <Text className="text-xs font-mono text-muted tabular-nums">00:47</Text>
          </View>

          <Text className="text-[15px] font-medium tracking-tight text-foreground mb-2">
            Drafting daily brief · step 3 of 5
          </Text>

          <View className="h-1 bg-border rounded-full overflow-hidden mb-2.5">
            <View className="h-full bg-blue-500 dark:bg-blue-400 rounded-full" style={{ width: '60%' }} />
          </View>
        ...
        <View className="bg-surface rounded-xl overflow-hidden">
          ...
          <Pressable className="flex-row items-center gap-3 px-4 py-3 active:opacity-70">
            ...
            <View className="flex-1 min-w-0">
              <Text className="text-sm font-medium text-foreground">Unlock front door · Dad</Text>
              <View className="flex-row items-center gap-1.5 mt-0.5 flex-wrap">
                <Text className="text-xs font-mono text-muted">apr_01JH7A4K</Text>
          ...
          <Pressable className="flex-row items-center gap-3 px-4 py-3 active:opacity-70">
            ...
            <View className="flex-1 min-w-0">
              <Text className="text-sm font-medium text-foreground">Intent unclear · garage SMS</Text>
              <View className="flex-row items-center gap-1.5 mt-0.5">
                <Text className="text-xs font-mono text-muted">esc_01JH7A9P</Text>
                <Text className="text-xs text-muted">·</Text>
                <Text className="text-xs text-muted">select interpretation</Text>
              </View>
            </View>
        ...
        <SectionHeader
          ...
          trailing={
            <Text className="text-xs font-mono text-blue-500 dark:text-blue-400">Tail →</Text>
          }
        />
        ...
        <View className="bg-surface rounded-xl px-3 py-1">
          {AUDIT_ROWS.map(([time, level, type, traceId], i) => (
            ...
            >
              <Text className="text-[10px] font-mono text-muted w-14">{time}</Text>
              <AuditLevelBadge level={level} />
              <View className="flex-1 flex-row items-center gap-1 min-w-0 overflow-hidden">
                <Text className="text-[10px] font-mono text-foreground shrink" numberOfLines={1}>
                  {type} ·{' '}
                </Text>
          ...
          {SUBSYSTEMS.map((sys, i) => (
            <View key={i}>
              <Pressable className="flex-row items-center gap-3 px-4 py-3 active:opacity-70">
                ...
                <View className="flex-1">
                  <Text className="text-sm font-medium text-foreground">{sys.name}</Text>
                  <View className="flex-row items-center gap-1.5 mt-0.5">
                    <StatusDot variant={sys.status === 'degraded' ? 'warning' : 'success'} />
                    <Text className="text-xs text-muted">{sys.status}</Text>
                    <Text className="text-xs text-muted">·</Text>
                    <Text className="text-xs font-mono text-muted">{sys.volume}</Text>
                  </View>
                </View>
                <SymbolView
              ...
              {i < SUBSYSTEMS.length - 1 && <View className="h-px bg-border mx-4" />}
            </View>
          ))}
        </View>
      </ScrollView>
